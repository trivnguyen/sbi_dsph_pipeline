...
def get_config():
    ...
    # seeding
    config.seed_data = 842
    config.seed_training = 852

    # data configuration
    config.data_root = '/scratch/tvnguyen/projects/sbi_twins/datasets/priorC_500k'
    config.data_name = 'ktwin'
    ...
    config.max_norm_graphs = 500_000

    ## LOGGING AND WANDB CONFIGURATION ###
    config.wandb_project = 'sbi_twins'
    config.workdir = '/scratch/tvnguyen/projects/sbi_twins/trained_models/ktwin'
    ...
    pre_transforms.uncertainty_args = [
        dict(
            distribution_type='jeffreys_varied',
            low_range=(0.01, 0.1),
            width_range=(5.0, 30.0),
            feature_idx=1
        )
    ]
    pre_transforms.selection_args = ConfigDict()
    pre_transforms.selection_args.selection_configs = [
        dict(type='radial', params=dict(dropout_min=0.0, dropout_max=0.75, mode='drop_outer')),
        dict(type='radial', params=dict(dropout_min=0.0, dropout_max=0.75, mode='drop_inner')),
        dict(type='radial', params=dict(dropout_min=0.0, dropout_max=0.75, mode='random')),
    ]
    ...
    config.num_epochs = scheduler.decay_steps
    config.num_steps = -1
    config.patience = 100
