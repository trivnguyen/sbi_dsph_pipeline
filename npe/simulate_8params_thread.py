""" Simulate the 6D stellar kinematics of dwarf galaxies (thread pool). """
...
# omp_set_num_threads() (called below via agama.setNumThreads) only
# configures OpenMP for the calling thread's context; OS threads
# later spawned by our own ThreadPoolExecutor can initialize their
# own OpenMP thread count (defaulting to the core count) the first
# time they enter agama's parallel regions. Setting OMP_NUM_THREADS
# before importing agama makes single-threaded OpenMP the
# process-wide default for every thread, avoiding thread-count
# explosion (n_workers x nproc) that can exhaust the OS thread
# limit. Must be set before `import agama`.
...
# We parallelize with our own Python-level ThreadPoolExecutor, so
# disable agama's internal OpenMP parallelism by default to avoid
# nested-parallelism thread-switching overhead and thread-count
# explosion (n_workers x nproc). The one exception is the locked
# sampling call in simulator() below, which temporarily raises this
# back up for its own duration - see _SAMPLE_LOCK.
agama.setNumThreads(1)

# agama.GalaxyModel.sample() draws from a single process-wide RNG
# (per agama's setRandomSeed docs), so concurrent calls from
# multiple threads race on that shared state - confirmed to reliably
# crash (heap corruption / thread exhaustion) under concurrent load
# without this lock. Serialize just the sampling step; potential/DF
# construction stays parallel across threads.
...
def simulator(
    num_stars: int, params: np.ndarray, sample_threads: int,
) -> np.ndarray | None:
    """
    ...
    Args:
        ...
        sample_threads: Number of OpenMP threads agama may use
            *inside* the locked sampling call. Safe to set high
            because holding `_SAMPLE_LOCK` already guarantees no
            other thread is concurrently inside agama, so there's
            no oversubscription risk from this call alone.
    ...
    try:
        ...
        with _SAMPLE_LOCK, agama.setNumThreads(sample_threads):
...
def simulate_one(
    params: np.ndarray, num_stars: int, sample_threads: int,
) -> tuple[np.ndarray | None, np.ndarray | None]:
    """
    Simulate and preprocess a single galaxy.

    Runs on a worker thread; the RNG-touching part of `simulator`
    is serialized via `_SAMPLE_LOCK` since agama's sampling draws
    from a single shared RNG.

    Args:
        params: Array of 8 model parameters.
        num_stars: Number of stars to sample for this galaxy.
        sample_threads: Passed through to `simulator`.
    ...
    Returns:
        ...
        preprocessing.
    """
    pv = simulator(num_stars, params, sample_threads)
    if pv is None:
        return None, None
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Simulate the 6D stellar kinematics of dwarf '
                     'galaxies (ThreadPoolExecutor version).')
    parser.add_argument(
    ...
    parser.add_argument(
        '--sample-threads', type=int,
        default=min(os.cpu_count() or 1, 16),
        help='Number of OpenMP threads agama may use inside the '
             'locked sampling call. Safe to set high regardless of '
             '--n-workers (see simulator() docstring). Tune this '
             'per-cluster.')
    parser.add_argument(
...
def main() -> None:
    ...
    if not args.append:
        file_idx = 0
        save_config(args, os.path.join(args.output_dir, 'config.0.json'))
    else:
    ...
    with ThreadPoolExecutor(max_workers=args.n_workers) as pool:
        pending = {
            pool.submit(simulate_one, p, n, args.sample_threads)
                ...
                for fut in done:
                    ...
                    if next_item is not None:
                        ...
                        pending.add(pool.submit(
                            simulate_one, p, n, args.sample_threads))
